...
def replace_last_linear(
    ...
    if last_linear_name is not None and last_linear_parent is not None:
        ...
        if reuse_previous_weights:
            previous_layer: nn.Linear = getattr(last_linear_parent, last_linear_name)
            previous_n_classes: int = previous_layer.out_features
            ...
            new_layer.weight.data[:previous_n_classes, :] = previous_layer.weight.data
